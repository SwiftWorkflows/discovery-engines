...
//  
//
//  Created by Hemant Sharma on 11/19/13.
//
//
...
void
GenerateHKLsFCC(int ringnos[],int nrings,int hkls[][4],int *nhkls)
{
#define N_HKLSFCC 58
...
main(int argc, char *argv[])
{
    ...
    char *str, dummy[1000];
    int LowNr, NrOrientations,nLayers;
    RealType Distance;
    ...
    rewind(fileParam);
    double Distances[nLayers],ExcludePoleAngle,LatticeConstant,Wavelength;
    RealType OmegaRanges[20][2], BoxSizes[20][4], px;
    int RingNumbers[20];
    int cntr=0,countr=0,nRings=0;
    ...
    fp = fopen("orientations.txt","r");
    if (fp==NULL){
        printf("Could not read file orientations.txt\n");
        return (1);
    }
    int count = 0;
    ...
    RealType RingRadii[nRings+1];
    RealType RingTtheta[20];
    int RingMult[20];
    int RingHKL[20][3];
    RealType **TheorSpots;
    int nTspots;
    ...
    FILE *ft;
    ft = fopen("DiffractionSpots.txt","w");
    FILE *fg;
    fg = fopen("Key.txt","w");
    FILE *fl;
    fl = fopen("OrientMat.txt","w");
    if (ft==NULL){
    ...
    for (j=0;j<NrOrientations;j++){
        ...
        CalcDiffrSpots_Furnace(OrientMatr, LatticeConstant, Wavelength, Distance, RingRadii, OmegaRanges, BoxSizes, NoOfOmegaRanges, ExcludePoleAngle, TheorSpots, &nTspots);
        fprintf(fg,"%i\n",nTspots);
        for (t=0;t<3;t++){
            for (p=0;p<3;p++){
